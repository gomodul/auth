...
// Provider is the implementation of `auth.Provider` for accessing OpenID Connect provider
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	if sess.IDToken == "" {
		return auth.User{}, fmt.Errorf("%s cannot get user information without id_token", p.providerName)
	}
	...
	if err != nil {
		return auth.User{}, fmt.Errorf("oauth2: error decoding JWT token: %w", err)
	}

	expiry, err := p.validateClaims(claims)
	if err != nil {
		return auth.User{}, fmt.Errorf("oauth2: error validating JWT token: %w", err)
	}
...
func (p *Provider) userFromClaims(claims map[string]interface{}, user *auth.User) {
	// required
	user.UserID = getClaimValue(claims, p.UserIdClaims)

	user.FullName = getClaimValue(claims, p.NameClaims)
	...
	user.RawData["location"] = getClaimValue(claims, p.LocationClaims)
}
