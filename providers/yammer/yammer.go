...
// Provider is the implementation of `auth.Provider` for accessing Yammer.
...
func populateUser(userMap map[string]interface{}, user *auth.User) error {
	user.Email = stringValue(userMap["email"])
	user.FullName = stringValue(userMap["full_name"])
	user.NickName = stringValue(userMap["full_name"])
	user.UserID = strconv.FormatFloat(userMap["id"].(float64), 'f', -1, 64)
	user.RawData["location"] = stringValue(userMap["location"])
	return nil
}
