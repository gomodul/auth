...
// Provider is the implementation of `auth.Provider` for accessing WeCom.
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	params := url.Values{}
	params.Add("access_token", user.Token.Access)
...
func (p *Provider) fetchUserID(session auth.Session, code string) (string, error) {
	sess := session.(*Session)
	params := url.Values{}
...
func userFromReader(reader io.Reader, user *auth.User) error {
	...
	user.RawData["location"] = obj.Address
