...
// Provider is the implementation of `auth.Provider` for accessing Xero.
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	user.FullName = apiResponse.Organisations[0].Name
	user.NickName = apiResponse.Organisations[0].LegalName
	user.RawData["location"] = apiResponse.Organisations[0].CountryCode
	user.RawData["description"] = apiResponse.Organisations[0].OrganisationType
	...
	user.Token.ExpiresAt = sess.AccessTokenExpires
	return user, err
}
