...
func Test_Implements_Provider(t *testing.T) {
	...
	a.Implements((*auth.Provider)(nil), xeroProvider())
}
...
func Test_FetchUser(t *testing.T) {
	...
	a.Equal("Vanderlay Industries", user.FullName)
	a.Equal("Vanderlay Industries", user.NickName)
	a.Equal("COMPANY", user.RawData["description"])
	a.Equal("111-11", user.UserID)
	a.Equal("NZ", user.RawData["location"])
	a.Empty(user.Email)
}
