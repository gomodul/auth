...
//
//	auth.UseProviders(dingTalkProvider)
//
...
// Provider is the implementation of `auth.Provider` for accessing DingTalk.
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	p.logDebug("Starting to fetch user info, AccessToken: %s", user.Token.Access[:10]+"...")
	...
	p.logDebug("Successfully retrieved user info: Name=%s, Email=%s", user.FullName, user.Email)
	return user, nil
}
...
func GetCorpID(user auth.User) (string, bool) {
	if user.RawData == nil {
		return "", false
	}
