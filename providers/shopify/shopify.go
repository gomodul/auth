...
// Provider is the implementation of `auth.Provider` for accessing Shopify.
...
func shopFromReader(r io.Reader, shop *auth.User) error {
	rsp := struct {
		Shop struct {
			ID              int64  `json:"id"`
	...
	shop.FullName = rsp.Shop.Name
	shop.Email = rsp.Shop.Email
	shop.RawData["description"] = fmt.Sprintf("%s (%s)", rsp.Shop.MyShopifyDomain, rsp.Shop.PlanDisplayName)
	shop.RawData["location"] = fmt.Sprintf("%s, %s", rsp.Shop.City, rsp.Shop.Country)
