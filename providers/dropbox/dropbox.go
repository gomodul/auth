...
// Provider is the implementation of `auth.Provider` for accessing Dropbox.
...
func userFromReader(r io.Reader, user *auth.User) error {
	...
	user.FullName = strings.TrimSpace(fmt.Sprintf("%s %s", u.Name.GivenName, u.Name.Surname))
	user.RawData["description"] = u.Name.DisplayName // Full name plus parenthetical team name
	user.Email = u.Email
	user.NickName = u.Email // Email is the dropbox username
	user.RawData["location"] = u.Country
