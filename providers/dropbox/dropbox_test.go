...
func Test_FetchUser(t *testing.T) {
	...
	a.Equal(user.FullName, "Franz Ferdinand")
	a.Equal(user.RawData["description"], "Franz Ferdinand (Personal)")
	a.Equal(user.NickName, "franz@dropbox.com")
	a.Equal(user.Email, "franz@dropbox.com")
	a.Equal(user.RawData["location"], "US")
	a.Equal(user.Token.Access, "1234567890")
	...
	// RawData contains 14 fields from API response + description + location = 16
	a.Len(user.RawData, 16)
}
