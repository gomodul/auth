...
// Provider is the implementation of `auth.Provider` for accessing Hubspot.
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	response, err := p.Client().Get(userEndpoint + url.QueryEscape(user.Token.Access))
	if err != nil {
		return user, err
	}
	...
	// Extract the user data we got from Google into our auth.User.
	user.Email = u.User
	...
	user.Token.ExpiresAt = accessTokenExpiration
