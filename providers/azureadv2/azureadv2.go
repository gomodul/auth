...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	user.Token.Access = msSession.AccessToken
	user.Token.Refresh = msSession.RefreshToken
	user.Token.ExpiresAt = msSession.ExpiresAt
	return user, err
}
...
func userFromReader(r io.Reader, user *auth.User) error {
	...
	user.RawData["location"] = u.OfficeLocation
	user.UserID = u.ID
