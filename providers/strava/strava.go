...
// Provider is the implementation of `auth.Provider` for accessing Strava.
...
func userFromReader(reader io.Reader, user *auth.User) error {
	u := struct {
		ID        int64  `json:"id"`
	...
	user.FullName = fmt.Sprintf("%s %s", u.FirstName, u.LastName)
	user.FirstName = u.FirstName
	user.LastName = u.LastName
	user.NickName = u.Username
	user.AvatarURL = u.Picture
	user.RawData["description"] = fmt.Sprintf(`{"gender":"%s"}`, u.Gender)
	user.RawData["location"] = fmt.Sprintf(`{"city":"%s","region":"%s","country":"%s"}`, u.City, u.Region, u.Country)
