...
// Provider is the implementation of `auth.Provider` for accessing naver.com.
...
func userFromReader(reader io.Reader, user *auth.User) error {
	...
	user.UserID = r.ID
	user.RawData["description"] = fmt.Sprintf(`{"gender":"%s","age":"%s","birthday":"%s"}`, r.Gender, r.Age, r.Birthday)
