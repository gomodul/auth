...
// Provider is the implementation of `auth.Provider` for accessing Steam
...
// buildUserObject is an internal function to build a auth.User object
// based in the data stored in r
func buildUserObject(r io.Reader, u auth.User) (auth.User, error) {
	...
	u.FullName = player.Name
	if len(player.Name) == 0 {
		u.FullName = "No name is provided by the Steam API"
	}
	u.NickName = player.NickName
	u.AvatarURL = player.AvatarURL
	u.Email = "No email is provided by the Steam API"
	u.RawData["description"] = "No description is provided by the Steam API"

	if len(player.LocationStateCode) > 0 && len(player.LocationCountryCode) > 0 {
		u.RawData["location"] = fmt.Sprintf("%s, %s", player.LocationStateCode, player.LocationCountryCode)
	} else if len(player.LocationCountryCode) > 0 {
		u.RawData["location"] = player.LocationCountryCode
	} else if len(player.LocationStateCode) > 0 {
		u.RawData["location"] = player.LocationStateCode
	} else {
		u.RawData["location"] = "No location is provided by the Steam API"
	}
