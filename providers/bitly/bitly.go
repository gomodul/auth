...
// Provider is the implementation of `auth.Provider` for accessing Bitly.
...
// Ensure `bitly.Provider` implements `auth.Provider`.
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	req.Header.Add("Authorization", fmt.Sprintf("Bearer %s", u.Token.Access))
...
func userFromReader(reader io.Reader, user *auth.User) (err error) {
	u := struct {
