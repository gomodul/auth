...
// Provider is the implementation of `auth.Provider` for accessing TikTok
...
func (p *Provider) GetClient() *http.Client {
	return auth.HTTPClientWithFallBack(p.Client)
}
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	if user.Token.Access == "" || user.UserID == "" {
...
func userFromReader(reader io.Reader, user *auth.User) error {
	...
	user.FullName = u.Data.DisplayName
