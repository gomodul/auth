...
// Provider is the implementation of `auth.Provider` for accessing Amazon.
...
// create one manually.
func New(clientKey, secret, callbackURL string, scopes ...string) auth.Provider {
	p := &Provider{
		ClientKey:    clientKey,
		Secret:       secret,
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	response, err := auth.HTTPClientWithFallBack(p.Client()).Get(endpointProfile + "?access_token=" + url.QueryEscape(sess.AccessToken))
...
func userFromReader(r io.Reader, user *auth.User) error {
	u := struct {
	...
	user.UserID = u.ID
	user.RawData["location"] = u.Location
	return nil
}
