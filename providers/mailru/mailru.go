...
// Provider is the implementation of `auth.Provider` for accessing MAILRU.
...
func (p *Provider) FetchUser(session auth.Session) (_ auth.User, err error) {
	var (
...
func (p *Provider) RefreshToken(refreshToken string) (*oauth2.Token, error) {
	...
	ts := p.oauthConfig.TokenSource(auth.ContextForClient(p.Client()), t)
