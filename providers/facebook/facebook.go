...
// Provider is the implementation of `auth.Provider` for accessing Facebook.
...
func (p *Provider) FetchUser(session auth.Session) (auth.User, error) {
	...
	// https://github.com/gomodul/auauthssues/96
...
func userFromReader(reader io.Reader, user *auth.User) error {
	...
	user.Email = u.Email
	user.RawData["description"] = u.About
	user.AvatarURL = u.Picture.Data.URL
	user.UserID = u.ID
	user.RawData["location"] = u.Location.Name
