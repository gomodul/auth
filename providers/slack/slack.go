...
// Provider is the implementation of `auth.Provider` for accessing Slack.
...
func simpleUserFromReader(r io.Reader, user *auth.User) error {
	u := struct {
...
func userFromReader(r io.Reader, user *auth.User) error {
	...
	user.FullName = u.User.Profile.Name
