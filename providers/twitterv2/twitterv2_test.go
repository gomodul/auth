...
func Test_Implements_Provider(t *testing.T) {
	...
	a.Implements((*auth.Provider)(nil), twitterProvider())
}
...
func Test_FetchUser(t *testing.T) {
	...
	a.Equal("Homer", user.FullName)
	a.Equal("duffman", user.NickName)
	a.Equal("Duff rules!!", user.RawData["description"])
	a.Equal("http://example.com/image.jpg", user.AvatarURL)
	a.Equal("1234", user.UserID)
	a.Equal("Springfield", user.RawData["location"])
	a.Equal("TOKEN", user.Token.Access)
	a.Equal("", user.Email)
}
